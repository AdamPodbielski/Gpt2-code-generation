...
# Modified by Adam Podbielski
# Training on python function and code to generate code based on instructions and input.
...
def main(test_mode=False):
    ...
    #######################################
    file_path = "code-functions-5k.json" #changed to my python dataset 5k sample size will bump to 10k later
...
def gen_text(text, num_new=256):
    '''
        generate new text based on text, up to num_new number of tokens
        ends due to <endoftext> token
    '''

    entry = {"instruction": text, "input": ""}
...
if __name__ == "__main__":
    ...
    gpt.load_state_dict(torch.load("gpt2-medium355M-sft-standalone.pth", map_location="cpu"))
    gpt.eval()
    tokenizer = tiktoken.get_encoding("gpt2")
